...
The `storage-volume-data-manager` module may stay enabled alongside `storage-foundation`. Both modules keep working: each one serves its own `DataExport` and `DataImport` resources independently — its own API group, its own namespace, and its own finalizers on the volumes involved. One and the same live volume (a `PersistentVolumeClaim` or a `VirtualDisk`), however, must be exported through only one of the two modules. There are two independent reasons for that:

- The public addresses coincide. Both modules build `status.publicURL` on the same host and from the same `/<namespace>/<target-kind>/<target-name>` path, so an export of a given volume gets the same public address no matter which of the two modules created it. If an export is deleted in one module and then created for the same volume in the other, the address is reused, and a client that remembers the address cannot tell which module's export answers it. Always read the address from `status.publicURL` of the resource you created yourself.
- Exporting a live volume takes its `PersistentVolume` over. The volume is temporarily rebound to the exporter, so the same volume cannot be exported through both modules at once: the export started second does not reach publication at all and ends with a conflict over the `PersistentVolume`. Delete the first export, or wait until it expires, before exporting the same volume through the other module.
