...
- Storage quota for `mode: PopulateData`: the staging volume is now allocated in the module namespace `d8-storage-foundation`, not the user's namespace — storage consumption counts against a `ResourceQuota` on `d8-storage-foundation` (if any) instead of the user's. Size it to at least the largest expected `spec.storageParams.size`, since an undersized quota leaves the internal PVC `Pending` forever and the import never becomes Ready. `mode: CreatePVC` is unchanged (the PVC stays in the user's namespace).
- Upgrade note: a `PopulateData` import that is mid-upload during a module upgrade to this version must be restarted (`d8 snapshot upload` again); partially uploaded data is discarded. Completed imports and their artifacts are unaffected. The staging PVC such an interrupted import left in your namespace is not reclaimed automatically — delete it manually; if it stays `Terminating`, remove its `storage-foundation.deckhouse.io/storage-manager-controller` finalizer.
