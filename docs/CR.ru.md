...
- Capture-домены на текущем SDK-контракте state-snapshotter создают или принимают VCR через
  `snapshotsdk.EnsureVolumeCapture`, публикуют его имя и управляют собственным lifecycle через
  `DomainCaptureStatus(...).Phase(...).Reason(...).Message(...).Apply(ctx)`. Обычные домены
  публикуют планы MCR/VCR до `PhasePlanned`; текущие subtree-gated агрегаторы могут публиковать
  собственный MCR после `PhasePlanned` как compatibility exception, пока ожидают сохранения
  манифестов потомков.
- Домены не читают conditions VCR и не пишут его status. Core state-snapshotter классифицирует
  терминальный VCR как `Ready=False/VolumeCaptureFailed` на snapshot-пути. Домен наблюдает
  core-owned результат через `snapshotsdk.CoreCaptureOutcome` и публикует `PhaseFinished` после
...
- Такое же empty-status ожидание возможно при невалидных `accessModes`, отсутствующем StorageClass
  или StorageClass другого provisioner. Часть путей публикует Event, часть игнорируется молча; ни
  один из них сейчас не передаёт VRR controller терминальный результат.
...
- Текущий writer `pvcRef` публикует `kind`, `namespace`, `name` и `uid`, но оставляет optional
  `apiVersion` пустым; пример намеренно совпадает с writer. Пока writer не начнёт публиковать это
  поле, consumers не должны считать его обязательным.
...
| Deckhouse `ClusterEditor` / RBAC v2 manager | Mutating verbs `create`, `update`, `patch`, `delete`, `deletecollection`; эффективный read-доступ наследуется через отдельно агрегированные viewer/User grants; grant на request `/status` отсутствует |
